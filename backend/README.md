...
For running, run 'python3 -m flask --app 'file_name' run'
...
Accounts are created and managed with Supabase auth. This means that the account email has to be at least somewhat legitimate (e.g. not just test@gmail.com) or Supabase will automatically reject the email. Furthermore, this means that you will not be able to see each account's passoword so, for testing purposes, just make each account's password ``password``.